...
package controllers.javascript {
  ...
  class ReverseShoppingCtrl(_prefix: => String) {
    ...
    // @LINE:26
    def viewCurrentOrders: JavaScriptReverseRoute = JavaScriptReverseRoute(
      "controllers.ShoppingCtrl.viewCurrentOrders",
      """
        function() {
          return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "viewCurrentOrders/"})
        }
      """
    )
